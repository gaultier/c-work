...
void brute_force(Point* points, uint32_t count, Point** a, Point** b,
                 double* distance);
void divide_conquer(Point* points, uint32_t count, Point* a, Point* b,
                    double* distance);
...
static void read_file(FILE* file, Point** points, uint32_t* size) {
    struct timeval t1;
    gettimeofday(&t1, NULL);
    int scanfed = 0;
    uint32_t count = 0;
    scanfed = fscanf(file, "%d\n", &count);
    printf("N: %d\n", count);
...
int cmp_x(const void* a, const void* b) {
    ...
    if ((ax - bx) < -DBL_EPSILON)
        return -1;
    else if ((ax - bx) > DBL_EPSILON)
        return 1;
    else
        return 0;
}
...
void divide_conquer(Point* points, uint32_t count, Point* a, Point* b,
    ...
    divide_conquer(points, middle_index, best_a_left, best_b_left,
                   &distance_left);

    Point* best_a_right = a;
    Point* best_b_right = b;
    double distance_right = DBL_MAX;
    divide_conquer(points + middle_index, count - middle_index, best_a_right,
                   best_b_right, &distance_right);
...
int main(int argc, char* argv[]) {
    char* filename = "/Users/pgaultier/code/c-work/100_000.txt";
    ...
    if (!file) {
        fprintf(stderr, "Error opening the file: %d %s\n", errno,
                strerror(errno));
        return errno;
    }

    read_file(file, &points, &count);
