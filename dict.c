...
uint32_t bnode_find(uint32_t parent_i, const char value[], uint8_t size,
                    const BNode nodes[]);
const char* word_find_naive(const String words[], uint32_t words_size, const char* word, uint8_t size, const char* content);
const char* word_find_divide(const String words[], uint32_t words_size, const char* word, uint8_t word_size, const char* content);
const char* word_find_memcmp(char* content, uint64_t count, const char* word, uint8_t word_size);
...
inline int8_t cmp(const char s1[], uint8_t size1, const char s2[], uint8_t size2) { 
...
const char* word_find_naive(const String words[], uint32_t words_size, const char* word, uint8_t word_size, const char* content)
{
...
const char* word_find_divide(const String words[], uint32_t words_size, const char* word, uint8_t word_size, const char* content)
{
  if (words_size <= 3) return word_find_naive(words, words_size, word, word_size, content);
  ...
  const int8_t compare = cmp(word, word_size, word_middle_str, word_middle->size);
  if (compare == 0) return word_middle_str;

  return compare < 0 ? word_find_divide(words, middle, word, word_size, content) : word_find_divide(word_middle + 1, words_size - middle, word, word_size, content);
}

const char* word_find_memcmp(char* content, uint64_t content_size, const char* word, uint8_t word_size) {
...
int main(int argc,const char *argv[]) {
  ...
  if (res_write == -1 || res_write != strings_size) fprintf(stderr, "[1] Error writing file: %s\n", strerror(errno));
  printf("[D010] %d\n", res_write);
  ...
  const char* found_naive = word_find_naive(words, count,  argv[2], arg_word_size, content);
  struct timeval time12;
  gettimeofday(&time12, NULL);
  printf("[Find naive] %p: %ld us\n", found_naive, (time12.tv_sec * 1000 * 1000 + time12.tv_usec) - (time11.tv_sec * 1000 * 1000 + time11.tv_usec));
  /////////////////////////////////////
  struct timeval time21;
  gettimeofday(&time21, NULL);
  const char* found_memcmp = word_find_memcmp(content, size, argv[2], arg_word_size);
  struct timeval time22;
  gettimeofday(&time22, NULL);
  printf("[Find memcmp] %p: %ld us\n", found_memcmp, (time22.tv_sec * 1000 * 1000 + time22.tv_usec) - (time21.tv_sec * 1000 * 1000 + time21.tv_usec));
  ...
  // printf("[Find tree] %d: %ld us\n",found_tree_i != UINT32_MAX, (time32.tv_sec * 1000 * 1000 + time32.tv_usec) - (time31.tv_sec * 1000 * 1000 + time31.tv_usec));
  /////////////////////////////////////
  struct timeval time41;
  gettimeofday(&time41, NULL);
  const char* found_divide = word_find_divide(words, count,  argv[2], arg_word_size, content);
  struct timeval time42;
  gettimeofday(&time42, NULL);
  printf("[Find divide] %p: %ld us\n", found_divide, (time42.tv_sec * 1000 * 1000 + time42.tv_usec) - (time41.tv_sec * 1000 * 1000 + time41.tv_usec));
  /////////////////////////////////////  
}
